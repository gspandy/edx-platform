...
@ddt.ddt
class TestComputeGrades(TestCase):
    """
    ...
    def setUp(self):
        super(TestComputeGrades, self).setUp()
        self.command = compute_grades.Command()

        self.user_ids = [user_id for user_id in range(self.num_users)]

        self.course_keys = []
        for course_index in range(self.num_courses):
            self.course_keys.append(CourseLocator(org='some_org', course='some_course', run=unicode(course_index)))

        self.subsection_keys_by_course = {}
        for course_key in self.course_keys:
            subsection_keys_in_course = []
            for sub_index in range(self.num_subsections):
                subsection_keys_in_course.append(
                    BlockUsageLocator(course_key=course_key, block_type='sequential', block_id=unicode(sub_index))
                )
            self.subsection_keys_by_course[course_key] = subsection_keys_in_course

    def test_course_selection(self):
        courses = self.command._iter_course_keys({'all_courses': True})
        self.assertEqual(list(courses), [1])
'''
